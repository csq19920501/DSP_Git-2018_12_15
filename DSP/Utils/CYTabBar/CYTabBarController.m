...
- (void)setSelectedIndex:(NSUInteger)selectedIndex{
    if (selectedIndex == 4) {
        if (_tabbar && [_tabbar.delegate respondsToSelector:@selector(tabbar:clickForCenterButton:)] ) {
            [_tabbar.delegate tabbar:_tabbar clickForCenterButton:nil];
        }
        return;
    }
    ...
    [_tabbar mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.bottom.right.equalTo(viewController.view);
...
-(void)csq_setCYTabBarHidden:(BOOL)hidden animated:(BOOL)animated{
    NSTimeInterval time = animated ? 0.3 : 0.0;
    if (!hidden) {
        self.tabbar.hidden = NO;
        [UIView animateWithDuration:time animations:^{
