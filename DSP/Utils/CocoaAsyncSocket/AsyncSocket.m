...
- (BOOL)acceptOnInterface:(NSString *)interface port:(UInt16)port error:(NSError **)errPtr
{
	...
	if (theSocket4)
	{
		...
		//MYLog(@"theSocket4: %hu", [self localPortFromCFSocket4:theSocket4]);
	}
	...
	if (theSocket6)
	{
		...
		//MYLog(@"theSocket6: %hu", [self localPortFromCFSocket6:theSocket6]);
	}
...
//        MYLog (@"AsyncSocket %p couldn't attach read stream to run-loop,", self);
//        MYLog (@"Error: %@", err);
...
//        MYLog (@"AsyncSocket %p couldn't attach write stream to run-loop,", self);
//        MYLog (@"Error: %@", err);
...
- (BOOL)openStreamsAndReturnError:(NSError **)errPtr
{
	...
	{
//        MYLog (@"AsyncSocket %p couldn't open read stream,", self);
        pass = NO;
	}
	
	if(pass && !CFWriteStreamOpen(theWriteStream))
	{
//        MYLog (@"AsyncSocket %p couldn't open write stream,", self);
        pass = NO;
	}
...
- (void)doStreamOpen
{
	if ((theFlags & kDidCompleteOpenForRead) && (theFlags & kDidCompleteOpenForWrite))
	{
		...
		{
//            MYLog (@"AsyncSocket %p couldn't get socket from streams, %@. Disconnecting.", self, err);
			[self closeWithError:err];
			return;
		}
...
- (BOOL)setSocketFromStreamsAndReturnError:(NSError **)errPtr
{
	...
	if(peeraddr == NULL)
	{
//        MYLog(@"AsyncSocket couldn't determine IP version of socket");
				  ...
				  withData:(const void *)pData
{
	...
	switch (type)
	{
		...
		case kCFSocketAcceptCallBack:
			...
			break;
		default:
//            MYLog(@"AsyncSocket %p received unexpected CFSocketCallBackType %i", self, (int)type);
			break;
	}
}
...
- (void)doCFReadStreamCallback:(CFStreamEventType)type forStream:(CFReadStreamRef)stream
{
	...
	switch (type)
	{
		...
		case kCFStreamEventEndEncountered:
			...
			break;
		default:
//            MYLog(@"AsyncSocket %p received unexpected CFReadStream callback, CFStreamEventType %i", self, (int)type);
            break;
	}
}
...
- (void)doCFWriteStreamCallback:(CFStreamEventType)type forStream:(CFWriteStreamRef)stream
{
	...
	switch (type)
	{
		...
		case kCFStreamEventEndEncountered:
			...
			break;
		default:
//            MYLog(@"AsyncSocket %p received unexpected CFWriteStream callback, CFStreamEventType %i", self, (int)type);
            break;
	}
}
