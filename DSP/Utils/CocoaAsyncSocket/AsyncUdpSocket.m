...
- (BOOL)canAcceptBytes:(CFSocketRef)sockRef
{
	...
	if(theNativeSocket == 0)
	{
//        MYLog(@"Error - Could not get CFSocketNativeHandle from CFSocketRef");
		return NO;
	} 
				  ...
				  withData:(const void *)pData
{
	...
	switch (type)
	{
		...
		case kCFSocketWriteCallBack:
			...
			break;
		default:
//            MYLog (@"AsyncUdpSocket %p received unexpected CFSocketCallBackType %lu.", self, (unsigned long)type);
			break;
	}
}
